...
Generating fake water for a fake sailboat
"""
...
class WaterGen(TestbedBase):

    name = "WaterGen"
    
    def __init__(self, settings=None): 
        super(WaterGen, self).__init__(settings=settings)
        dimensions = [30,30]
        ...
        fixtureA = b2d.fixture_def(shape=b2d.circle_shape(1),density=2.2, friction=1, restitution=0.5)
        body = self.world.create_dynamic_body(
            position=(13,10),
            fixtures=fixtureA
        ) 

        pdef = b2d.particle_system_def(viscous_strength=2,spring_strength=0.0, damping_strength=1,pressure_strength=0.8,
                                     color_mixing_strength=0.008,density=5)
